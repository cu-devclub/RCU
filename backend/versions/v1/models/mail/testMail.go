...
func (m *MailModel) SendMail() error {
	client, err := m.InitSMTP()
	if err != nil {
		log.Fatalf("Failed to initialize SMTP client: %v", err)
	}

	to := "6634473123@student.chula.ac.th"

	now := time.Now()
	formattedTime := now.Format("2006-01-02 15:04:05")
	subject := fmt.Sprintf("[หอพักนิสิตจุฬาฯ] คำขอรีเซ็ตรหัสผ่านจาก IP xxx.xxx.xxx.xxx - %s (UTC+7)", formattedTime)
	htmlBody := `<html>
<head>
  <meta charset="UTF-8">
  ...
  <table align="center" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px; margin:auto; background-color:#ffffff; border-radius:6px; overflow:hidden;">
    ...
    <tr>
      <td style="padding:30px;">
        ...
        <p style="font-size:28px; font-weight:bold; letter-spacing:2px; color:#1b1f2a;">XXXXXX</p>

		<p style="color:#999;">Ref: XXXX</p>
